...
describe('resolveActiveSleepWindow', () => {
  it('uses upcoming window when apply is before bedtime', () => {
    const now = localDate(2026, 5, 1, 20, 30);
  ...
  it('progresses during the window', () => {
    ...
    const now = localDate(2026, 5, 2, 2, 30);
    const w = resolveActiveSleepWindow(now, BED, WAKE);
...
const sleepWindowGolden = JSON.parse(
  fs.readFileSync(path.join(__dirname, '..', '..', 'shared', 'sleep_window_golden.json'), 'utf8'),
);

describe('sleep window golden', () => {
  it('matches shared fixtures', () => {
    expect(sleepWindowGolden.intervalMinutes).toBe(INTERVAL_MINUTES);
    for (const case_ of sleepWindowGolden.cases) {
      expect(sleepWindowDurationMinutes(case_.bedtimeMinutes, case_.wakeMinutes)).toBe(
        case_.durationMinutes,
      );
      expect(intervalCountForWindow(case_.bedtimeMinutes, case_.wakeMinutes)).toBe(
        case_.intervalCount,
      );
    }
  });
